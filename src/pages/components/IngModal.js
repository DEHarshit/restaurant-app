...
export default function IngModal({ isVisible, mode, setModal, setMode, ingredient, setIngredients, ind }) {
        ...
        if (name === ingredient.NAME && type === ingredient.TYPE && uom === ingredient.UOM) {
            ...
            async function editIngredient() {
                const postData = {
                    ...
                    body: JSON.stringify({ id: id, name: name, type: type, uom: uom })
                }
        ...
        if (name === '' || type === '' || uom === '') {
            ...
            async function addIngredient() {
                const postData = {
                    ...
                    body: JSON.stringify({name: name, type: type, uom: uom })
                }
                const res = await fetch(`${process.env.NEXT_PUBLIC_URL}/api/ingredients`, postData);
                const response = await res.json();
                setIngredients(response.ingredients)
