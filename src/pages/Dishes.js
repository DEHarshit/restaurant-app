...
export default function Dishes() {
  ...
  async function getDishes() {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_URL}/api/dishes`,
      postData
    );
    const response = await res.json();
  ...
  return (
    <div className="flex bg-black justify-center w-screen ">
        ...
        <div className="flex items-center justify-between w-full">
          ...
          <div className="flex space-x-5 px-10">
            <button
              ...
              type="button"
              className="bg-zinc-400 hover:text-white text-lg text-black p-2 px-10 font-semibold transition-all duration-400 hover:text-xl rounded-full hover:bg-gradient-to-r from-[#CEA07E] to-[#BB5656]"
            >
      ...
      <div>
        <SpecialModal isVisible={modal} setModal={setModal} dishes={dishes} />
      </div>
    </div>
  );
}
