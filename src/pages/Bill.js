...
export default function Bill() {
  ...
  async function getOrders() {
    if (id) {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_URL}/api/bill`,
        postData
      );
  ...
  async function getPhone() {
    if (session) {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_URL}/api/getphone`,
        postData
      );
      const response = await res.json();
      if (response) {
    ...
    return (
      <div>
        <div className="h-screen flex items-center justify-center">
          <div
            id="billContent"
            className="flex flex-1 flex-col bg-zinc-900 max-h-[700px] overflow-auto max-w-[750px] rounded-xl"
          >
                        ...
                        <tbody key={index}>
                          <tr className="hover:bg-opacity-10 hover:bg-white">
                            <td className="py-4 px-4">
                              <div className="flex">{ele.DNAME}</div>
                            </td>
                            <td className="py-4 px-4">
                              <div className="flex">{ele.QTY}</div>
                            </td>
                            <td className="py-4 px-4">
                              <div className="flex">{ele.PRICE}</div>
                            </td>
                          </tr>
                        </tbody>
                      ))}
              ...
              <div className="flex items-center justify-between">
                {order.PSTATUS !== "Paid" ? (
                  <div className="flex items-center space-x-4">
                    <div className="flex font-semibold text-2xl">Payment:</div>
                    <button
                      onClick={handlePayment}
                      type="button"
                      className="w-[150px] bg-zinc-700 hover:bg-blue-700 p-2 rounded-full px-5 hover:scale-[1.1] transition-all flex items-center justify-center space-x-1"
                    >
                      Pay
                    </button>
                  </div>
                ...
                <div>
                  {order.PSTATUS === "Paid" ? (
                    <button
                      onClick={generatePDF}
                      type="button"
                      className="w-[150px] bg-zinc-700 hover:bg-pink-700 p-2 rounded-full px-5 hover:scale-[1.1] transition-all flex items-center justify-center space-x-1"
                    >
                      Generate Bill
                    </button>
                  ) : (
                    <div className="flex font-semibold text-2xl">Not Paid</div>
                  )}
                </div>
              </div>
            </div>                                                                                                                                                                                                                                                                                               
