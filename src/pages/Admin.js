  ...
  return (
    <div className="flex flex-col h-dvh font-primary tracking-wider">
      <div className="flex items-center justify-between py-5 px-12 bg-zinc-900 border-b border-zinc-700">
        ...
        <div className="flex space-x-3 items-center">
          ...
          <button
            onClick={signOut}
            type="button"
            className="scale-[0.9] w-fit h-fit bg-zinc-400 hover:text-white text-md text-black p-2 font-semibold transition-all duration-400 rounded-full hover:bg-gradient-to-r from-[#CEA07E] to-[#BB5656]"
          >
            Sign Out
          </button>
        </div>
      ...
      <div className="flex bg-zinc-900 w-screen overflow-hidden">
        <div className="h-screen flex flex-col border-r border-zinc-700 ">
          {" "}
          {/* left side */}
          {menu.map((ele, index) => {
            return ele.role.includes(session?.user?.image) ? (
              <div
                className={`hover:bg-zinc-700 flex w-[200px] h-[75px] bg-zinb-900 border-b border-zinc-700 transition-all  items-center justify-center`}
              >
                <button
                  className={`${
                    ele.id === freeze
                      ? "text-xl font-semibold bg-gradient-to-r from-[#CEA07E] to-[#BB5656] inline-block text-transparent bg-clip-text"
                      : "text-white hover:font-semibold hover:text-lg hover:bg-gradient-to-r from-[#CEA07E] to-[#BB5656] inline-block hover:text-transparent bg-clip-text"
                  } h-full w-full transition-all `}
                  type="button"
                  onClick={() => handlePage(ele.id)}
                >
                  {ele.title}
                </button>
              </div>
        ...
        <div className="overflow-auto flex bg-black flex-grow">
          {" "}
          {/* page */}
          {currentPage}
        </div>
